...
class DB:
	...
	def list_snapshots(self) -> Res:
		"""List snapshots"""
		cur = self.db.exec("""SELECT * FROM snapshot;""")
		return Res.decode(cur, cur.fetchall())

	def read_snapshot(self, time: datetime.datetime) -> Res:
		"""
		...
		from res
		join side:inner latest_snapshots (res.snapshot == latest_snapshots.id)
		```
		"""
		...
		cur = self.db.exec(
			dedent(
				...
				FROM
					res
					JOIN latest_snapshots ON res.snapshot = latest_snapshots.id
